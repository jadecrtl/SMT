...
(*Fonction auxiliaire permettant de créer une liste de Var : *) 
...
let smtlib_of_wa p = 
  ...
  let loop_condition p =
    ...
    ^str_assert_forall (p.nvars) ("=> (and "^str_condition(create_array(p.nvars))^" "^str_of_test(p.loopcond)^") "^str_condition(p.mods)^")")       
